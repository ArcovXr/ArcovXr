...
function Project_card({ project, description, image, technologies, link }: Props) {
  ...
  const handleClick = () => {
    if (link.startsWith("http")) {
      ...
      navigate(link); // ✅ así se hace con React Router
    }
  };
  ...
  return (
    <div className="lg:flex w-full lg:w-15/16 lg:h-100 shadow-sm hover:shadow-lg transition-all shadow-gray-500 rounded-[35px] bg-gradient-to-b justify-between">
      <div className="transition-all lg:w-2/5 lg:p-0 p-2 hover:cursor-pointer">
        ...
        >
          <div className="transition-all lg:rounded-none rounded-4xl bg-black opacity-50 hover:opacity-30 h-full w-full lg:rounded-l-4xl">
      ...
      <div className="lg:w-3/5 lg:p-10 p-5 flex flex-col justify-between h-full">
        <div>
          ...
          <div className="font-light text-sm text-gray-200 lg:mt-10 mt-5 justify-center">{description}</div>
        </div>
